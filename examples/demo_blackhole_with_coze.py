"""
黑洞训练 - 加入扣子AI全家桶
"""

import sys
import os
import json

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from xuni import BlackHoleTrainer, MultiverseResourceFactory


def main():
    print("\n🌌 黑洞追加吞噬 — 扣子AI（Coze）全家桶...\n")

    # 收集所有代码库
    all_repos = []
    workspace = "/workspace"
    skip_dirs = [".git", "node_modules", "__pycache__", "venv", ".venv", "dist", "build", "kosong", "coze_temp"]

    for item in os.listdir(workspace):
        full_path = os.path.join(workspace, item)
        if os.path.isdir(full_path) and item not in skip_dirs:
            has_code = False
            for root, dirs, files in os.walk(full_path):
                dirs[:] = [d for d in dirs if not d.startswith(".") and d not in skip_dirs]
                for f in files:
                    if f.endswith((".py", ".js", ".ts", ".go", ".rs", ".c", ".cpp", ".h", ".java")):
                        has_code = True
                        break
                if has_code:
                    break
            if has_code:
                all_repos.append(full_path)

    # 加上 coze_repos 目录下的子仓库
    coze_dir = os.path.join(workspace, "coze_repos")
    if os.path.isdir(coze_dir):
        for item in os.listdir(coze_dir):
            full_path = os.path.join(coze_dir, item)
            if os.path.isdir(full_path) and not item.startswith("."):
                all_repos.append(full_path)

    print(f"共 {len(all_repos)} 个代码库进入黑洞")

    trainer = BlackHoleTrainer(model_id="xenith-ultimate-coze-blackhole")
    factory = MultiverseResourceFactory()

    result = trainer.absorb_and_forge(
        repo_paths=all_repos,
        factory=factory,
        languages=None,
        max_files_per_repo=10000,
        spin_rounds=15,
        quality_threshold=0.5,
        knowledge_domains=[
            "computer_science", "engineering", "math",
            "physics", "chemistry", "biology",
            "medicine", "law", "finance", "philosophy",
        ],
        knowledge_count_per_domain=15000,
    )

    report_path = "/workspace/xuni/examples/blackhole_with_coze_report.json"
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2, default=str)

    print(f"\n报告已保存：{report_path}\n")

    return result


if __name__ == "__main__":
    main()
