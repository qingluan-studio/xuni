"""
demo_trainer_v5.py —— 逆天而行 v5：8 大仓库 + 100000 轮

觅食清单（8 个仓库）：
  1. CPython 标准库    (868 文件)
  2. Django Web 框架   (762 文件)
  3. scikit-learn ML   (616 文件)
  4. pandas 数据科学   (284 文件)
  5. pydantic 验证     (110 文件)
  6. flask Web         (33 文件)
  7. click CLI         (29 文件)
  8. requests HTTP     (20 文件)
  + 工厂自身代码

训练: 100000 轮 × 20 条/轮 = 2,000,000 片段

运行:
  cd /workspace/xuni
  python examples/demo_trainer_v5.py
"""
...
def main():
    print("=" * 60)
    print("  🔥🔥 xuni v5 —— 逆天而行：8 大仓库 + 100000 轮")
    print("=" * 60)
    ...
    baseline_prompts = [
        ...
        "class Model", "def fit", "class DataFrame",
        "def groupby", "class QuerySet",
    ]
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10000 == 0:
            ...
            print(f"  Epoch {epoch+1:6d} | 已学: {learned:12,d} | "
    ...
    # 精简报告（不存完整 comparison，避免 JSON 过大）
    report = {
        "version": "v5",
        ...
        "comparison_samples": {
            p: {"before": baseline[p][:80], "after": model._lite.generate(p, max_new_tokens=50)[:80]}
            for p in baseline_prompts[:5]  # 只存前 5 个示例
        },
    }

    report_path = os.path.join(os.path.dirname(__file__), "trainer_v5_report.json")
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2, default=str)
    print(f"  报告: {report_path}")

    # 精简元信息
    meta_path = os.path.join(os.path.dirname(__file__), "checkpoints", "harmonia_v5_meta.json")
    ...
    print(f"""
  📦 8 大仓库:
    CPython(868) + Django(762) + sklearn(616) + pandas(284)
    + pydantic(110) + flask(33) + click(29) + requests(20)
