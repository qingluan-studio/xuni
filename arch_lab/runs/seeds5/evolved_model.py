...
class Node2Op(nn.Module):
    """深度可分离卷积(k=1) + BN + nn.SiLU + 残差"""
    def __init__(self, c=32):
        super().__init__()
        self.dw = nn.Conv2d(c, c, 1, padding=0, groups=c, bias=False)
...
class EvolvedModel(nn.Module):
    ...
    def __init__(self, in_channels=1, num_classes=10, c=32):
        ...
        self.nodes = nn.ModuleList([Node0Op(c), Node1Op(c), Node2Op(c), Node3Op(c), Node4Op(c), Node5Op(c), Node6Op(c), Node7Op(c)])
    ...
    def forward(self, x):
        ...
        outs[2] = self.nodes[2](outs[1] + outs[-1] + outs[0])
        outs[3] = self.nodes[3](outs[2])
        outs[4] = self.nodes[4](outs[3] + outs[2])
        outs[5] = self.nodes[5](outs[4] + outs[3])
        outs[6] = self.nodes[6](outs[5])
        outs[7] = self.nodes[7](outs[5])
        feat = self.pool(outs[6] + outs[7]).flatten(1)
